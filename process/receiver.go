...
var RangeConcurrent_num = ut.RangeConcurrent
var RangeConcurrent_ch = ut.RangeConcurrent_ch
...
var PartialSize = ut.PartialSize
...
func GetProxy_http(ip_to string, head string, receiveBody_t ReceiveBody) {
	...
	buf := make([]byte, 8192)
	log.Debugf("GetProxy_http Read_time_out: %d|| url_id: %s|| origin_remote_ip: %s|| url: %s", Read_time_out, receiveBody_t.Url_id, receiveBody_t.Origin_Remote_IP, receiveBody_t.Url)
	tcpconn.SetDeadline(time.Now().Add(time.Duration(Read_time_out) * time.Second))
...
func GetProxy_https(ip_to string, head string, receiveBody_t ReceiveBody) {
	...
	buf := make([]byte, 8192)
	log.Debugf("GetProxy_https Read_time_out: %s|| url_id: %s|| origin_remote_ip: %s|| url: %s", Read_time_out, receiveBody_t.Url_id, receiveBody_t.Origin_Remote_IP, receiveBody_t.Url)
	tcpconn.SetDeadline(time.Now().Add(time.Duration(Read_time_out) * time.Second))
...
func Process_go(body ReceiveBody) {
	...
	log.Debugf("Process_go Url_id: %s|| origin_remote_ip: %s|| url: %s|| Ip_to: %s|| Head_send: %s|| is_http_t: %t", body.Url_id, body.Origin_Remote_IP, body.Url, Ip_to, Head_send, is_http_t)
	if is_http_t == true {
...
func RangeTask(body ReceiveBody) {
	...
	_, target_total_length, http_status = ProxyRangeRequest(Ip_to, Head_send, body.Url_id, body.Url, 10, 0, 1, target_total_length)
	...
	for i := 1; i <= loop_num; i++ {
		if i == 1 && target_total_length < PartialSize { // 文件大小 < 分片大小
			go func(n int) {
				read_length, _, _ = ProxyRangeRequest(Ip_to, Head_send, body.Url_id, body.Url, 10, (n-1)*PartialSize, target_total_length-1, target_total_length)
			...
			go func(n int) {
				read_length, _, _ = ProxyRangeRequest(Ip_to, Head_send, body.Url_id, body.Url, 10, (n-1)*PartialSize, (n-1)*PartialSize+last_length-1, target_total_length)
				range_ch <- read_length
			...
			go func(n int) {
				read_length, _, _ = ProxyRangeRequest(Ip_to, Head_send, body.Url_id, body.Url, 10, (n-1)*PartialSize, n*PartialSize-1, target_total_length)
				range_ch <- read_length
